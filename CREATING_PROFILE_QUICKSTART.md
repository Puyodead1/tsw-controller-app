...
![Calibration Tab](./images/creating-a-profile-quickstart/001-calibration-tab.png)  
...
![Calibration Dialog](./images/creating-a-profile-quickstart/002-calibration-tab-calibration-dialog.png)  
...
```json
{
  "$schema": "https://raw.githubusercontent.com/LiamMartens/tsw-controller-app/refs/heads/main/website/profile-builder-json-schema/profile.complete.schema.json",
  "name": "",
  "controls": []
}
```  
...
![Create new profile option](./images/creating-a-profile-quickstart/005-create-new-profile-option.png)  
![Empty profile builder](./images/creating-a-profile-quickstart/006-new-profile-builder.png)  
...
![First control](./images/creating-a-profile-quickstart/007-profile-builder-first-control.png)  
...
![First assignment](./images/creating-a-profile-quickstart/008-first-assignment.png)  
...
![Throttle Control](./images/creating-a-profile-quickstart/010-throttle-control.png)  
  
Next we can press "Add item" in the assignments section of our throttle control section, however instead of selecting "Momentary" we will select direct control. Some optional fields from the momentary assignment type may still be shown 

In order to configure any of the three "control" assignment types we will need to launch the game since we need to use the cab debugger to figure out the actual input values of the in-game controls. So, launch the game, and load up the training center. From there we will spawn in the ALP-45 DP light loco and take control of the locomotive. (for the cab debugger to work you need to have taken control of the locomotive). Now you can switch the controller app to the "Cab Debugger" tab. If you have the HTTP API enabled all the available controls should show up right away. If you are not using the API, the controls will only be displayed once you interact with them. In the example below, the API is enabled and so we can see all the available locomotive controls and their current state.  
...
Now let's figure out the values of the master controller in the game. Start by moving the handle to the maximum dynamic brake position, we will assume this is the minimum possible value. We can note down the value